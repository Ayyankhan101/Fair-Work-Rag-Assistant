...
def md_to_documents(md_path: str) -> list[Document]:
    ...
    documents = []
    # DEF-052: Compute source hash for provenance
    ...
    for section in sections:
        ...
        for i, chunk in enumerate(chunks):
            ...
            metadata = {
                ...
                'source_url': f"https://www.fairwork.gov.au/employment-conditions/awards/{source_file.replace('.pdf', '').upper()}",
                'document_type': 'Award',
...
def ingest_from_md(md_dir: str = "data/md_awards") -> list[Document]:
    ...
    if errors:
        ...
        raise SystemExit(f"Ingestion failed: {len(errors)} source(s) rejected")
